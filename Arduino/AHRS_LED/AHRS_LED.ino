...
#define GYRO_AVERAGE_OFFSET_X ((float) -16.6)
#define GYRO_AVERAGE_OFFSET_Y ((float) 36.5)
#define GYRO_AVERAGE_OFFSET_Z ((float) 26.4)
...
void loop()
{
  ...
  //LED gets brighter with higher acceleration
  //Remove low values/noise due to vibrations
  int modifiedAccel[3];
  int absAccel[3];
  absAccel[0] = abs(accel[0]);
  if(absAccel[0] < 40)
  {
    modifiedAccel[0] = 0;
  }
  ...
  {
    modifiedAccel[0] = map(absAccel[0],39, 100, 0, 100);
  }
  
  int currentBrightness = map(modifiedAccel[0], 0, 100, 8, 128);
  LEDS.showColor(CRGB(currentBrightness, 0, 0));
  delay(5);
